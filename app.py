...
Sections:
  ...
  2. Per-Hit Latency      — Box plot (Baseline vs FlashAgent) + stats table
...
st.set_page_config(page_title="FlashAgent Dashboard", page_icon="⚡", layout="wide")

# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.header("Data")
    baseline_path = st.text_input("Baseline CSV", "data/base_custom_telemetry.csv")
    cached_path   = st.text_input("FlashAgent CSV",      "data/apc_custom_telemetry.csv")
...
if not df_cache.empty:
    ...
    # Cumulative series
    df_cum = df_cmp[["query_index", "lat_base", "lat_apc", "cost_base", "cost_apc", "tok_base", "tok_apc"]].copy()
    ...
    df_cum["cum_tok_base"]  = df_cum["tok_base"].cumsum()
    df_cum["cum_tok_apc"]   = df_cum["tok_apc"].cumsum()
...
# ── Title ─────────────────────────────────────────────────────────────────────
st.title("⚡ FlashAgent Dashboard")
st.caption(
    "Real-time impact of the FlashAgent system on the **FinBench custom dataset**. "
    f"Baseline: **{n_total} queries** | "
    f"FlashAgent run: **{n_done} / {n_total}** queries processed."
)
st.progress(
    min(1.0, n_done / n_total) if n_total else 0.0,
    text=f"FlashAgent run progress: **{n_done} / {n_total}** queries",
)
...
# ══════════════════════════════════════════════════════════════════════════════
# 2. Per-Hit Latency: Baseline vs FlashAgent
# ══════════════════════════════════════════════════════════════════════════════
st.divider()
st.header("2. Per-Hit Latency: Baseline vs FlashAgent")
...
if not df_hits.empty:
    ...
    st.markdown(
        ...
        f"(**~{df_hits['lat_apc'].mean():,.0f} ms**) — a **{speedup:.0f}× speedup** with FlashAgent."
    )

    col_box1, col_box2 = st.columns(2)
    for col_st, label, data_col, color in [
        (col_box1, "Baseline",  "lat_base", BASELINE_COLOR),
        (col_box2, "FlashAgent (HIT)", "lat_apc",  CACHED_COLOR),
    ]:
...
if not df_hits.empty:
    ...
    st.markdown(
        f"FlashAgent cache hits consume **0 tokens** (blueprint retrieved from vector index). "
...
if not df_cum.empty:
    ...
    with col_g:
        ...
        fig_cl.add_trace(go.Scatter(
            ...
            name="FlashAgent", line=dict(color=CACHED_COLOR, width=2),
        ...
        fig_cl.update_layout(
            title="Cumulative latency: Baseline vs FlashAgent",
            xaxis_title="Query index",
    ...
    with col_h:
        final_tok_base  = float(df_cum["cum_tok_base"].iloc[-1])
        final_tok_apc   = float(df_cum["cum_tok_apc"].iloc[-1])
        tok_pct_saved   = (final_tok_base - final_tok_apc) / final_tok_base * 100 if final_tok_base else 0.0
        fig_ct = go.Figure()
        fig_ct.add_trace(go.Scatter(
            x=df_cum["query_index"], y=df_cum["cum_tok_base"],
            name="Baseline", line=dict(color=BASELINE_COLOR, width=2),
        ))
        fig_ct.add_trace(go.Scatter(
            x=df_cum["query_index"], y=df_cum["cum_tok_apc"],
            name="FlashAgent", line=dict(color=CACHED_COLOR, width=2),
            fill="tonexty", fillcolor="rgba(74,144,217,0.10)",
        ))
        fig_ct.add_annotation(
            x=df_cum["query_index"].iloc[-1],
            y=final_tok_apc,
            text=f"Saved {tok_pct_saved:.1f}% tokens",
            showarrow=True, arrowhead=2, ax=-80, ay=-30,
            font=dict(color="grey", size=11),
        )
        fig_ct.update_layout(
            title="Cumulative token usage: Baseline vs FlashAgent",
            xaxis_title="Query index",
